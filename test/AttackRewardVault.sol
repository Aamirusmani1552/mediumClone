  ...
  TypeAndVersionInterface
{
  ...
  // @audit-info in short, make some checks, update vestedRewardPerToken, update RewardBuckets rate
  // and amount and transfer LINK from rewarder address to this address
  function addReward(
    address pool,
    uint256 amount,
    ...
    // @audit-info pool == address(0) add to all pools
    // @audit-info pool == address(specificPool) add to only specific pool
    if (
    ...
    // @audit-info checks if both are not zero and not less than delegationRewardRate
    // to make sure the added amount will not be rounded to zero
  ...
  function claimReward() external override whenNotPaused returns (uint256) {
    ...
    // @audit-info it does the following things:
    // 1. add the rewards in staker.storedBaseReward and finalizedDelegatedReward
    // 2. calculate the delegated rewards
    // 3. update staker baseRewardPerToken
    // 3. update staker delegatedRewardPerToken if an operator
  ...
  function finalizeReward(
    ...
    // @audit-info cannot claim when paused
    if (paused() && shouldClaim) revert CannotClaimRewardWhenPaused();

    // @audit-info if this is called when staking first time, it will not have any effect on the
    // vested reward per token for any bucket
    _updateRewardPerToken();
    ...
    // @audit-info it does the following things
    // 1. calculate multiplier. if multiplier and apply it of earned amounts
    // 2. add rewards to staker.earnedBaseRewardInPeriod and staker.finalizedBaseReward
    // 3. if the amount will be forfeited then the staker.storedBaseReward = 0
    //    else staker.storedBaseReward = forfeited rewards
    // 4. return the forfeited amount
  ...
  // @audit-info if this has been called means new VestedRewardPerTokenHasBeen updated already
  // in the pools
  ...
  function _getBucketRewardAndEmissionRateSplit(
    ...
    // @audit-info WAD = 10e18
    // @audit-info divWadDown Equivalent to (x * WAD) / y rounded down.
    // @audit-info mulWadDown Equivalent to (x * y) / WAD rounded down.
    // @audit-info formula using:
    //    (amount * operatorPoolShare / 10e18) * 10e18 / totalPoolShare
  ...
  function _checkForRoundingToZeroRewardAmountSplit(
    ...
    if (
      ...
      && (
        (
          operatorPoolShare != 0 // Equivalent to (x * y) / WAD rounded down.
            && rewardAmount.mulWadDown(operatorPoolShare) * FixedPointMathLib.WAD < totalPoolShare
        )
          || (
            communityPoolShare != 0 // Equivalent to (x * y) / WAD rounded down.
  ...
  function _updateRewardPerToken() private {
    ...
    if (s_rewardPerTokenUpdatedAt < block.timestamp) {
      ...
      // @audit-info no need to change the tokens are provided at the smae time
  ...
  function _calculatePoolsRewardPerToken() private view returns (uint256, uint256, uint256) {
    // @audit-info _getTotalPrincipa() returns the current total amount staked in a pool or when
    // it was migrated
  ...
  // @audit-info basically only calculate new VestedRewardPerToken when the totalpricipal is greater
  // than zero and the time at which the rewards are added is not some like the previous token added
  // time
  // @audit-info s_rewardPerTokenUpdatedAt should be updated later
  function _calculateVestedRewardPerToken(
    RewardBucket memory rewardBucket,
    uint256 totalPrincipal
  ) private view returns (uint256) {
    // @audit-info amount stored in pool is zero, return old vestedRewardPerToken (should be 0)
    // @audit what if total principal has been withrawn, what would be the vertedRewardPerToken
    if (totalPrincipal == 0) return rewardBucket.vestedRewardPerToken;

    // @audit-info will be equal to either reward finished time or block.timestamp whatever is less
    uint256 latestRewardEmittedAt = Math.min(rewardBucket.rewardDurationEndsAt, block.timestamp);

    // @audit-info if the current time at which the rewards are added is less than or equal to the
    // previous token added time, then return the previous vestedRewardPerToken
    // @audit-info if updated at the same time return old vestedRewardPertoken
    ...
    // @audit-info Equivalent to (x * WAD) / y rounded down.
    // WAD = 1e18
    return rewardBucket.vestedRewardPerToken
      + (elapsedTime * rewardBucket.emissionRate).divWadDown(totalPrincipal); // @audit-info total
      // rewards will be used to transfer only he token equals to the totalPrincipal of a pool
  }
  ...
  function _applyMultiplier(
    ...
    // @audit-info this claimedBaseRewardInPeriod should be reset later
    // @audit-info this is calculating rewards if full multiplier is reached
    ...
    // @audit-info this is calculating for the current multiplier time only
    ...
    // @audit will there be any forfeited rewards if the multiplier is say 0.9 or very close to 1
  ...
  function _calculateStakerReward(
    ...
    // @audit-info storedBaseReward -> rewards from last stake / unstake till now
    ...
    if (isOperator) {
      ...
      // @audit-info finalizedDelegatedReward -> totalDelegatedRewardsEarned upto last stake /
      // unstake
  ...
  function _getUnvestedRewards(RewardBucket memory bucket) private view returns (uint256) {
    // @audit-info if the rewardDuration Has ended then the rewards will be 0 otherwise remaining
